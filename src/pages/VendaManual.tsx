...
import { ArrowLeft, Plus, Minus, Search, Store, ShoppingCart, Calendar, CreditCard, Banknote, QrCode, Trash2, X } from "lucide-react";
    ...
    <div className="bg-background-light dark:bg-background-dark font-display antialiased text-slate-900 dark:text-white min-h-screen flex flex-col">
      ...
      <main className="flex-1 overflow-y-auto pb-32 max-w-4xl mx-auto w-full">
          ...
          <div className="space-y-3">
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-3 bg-slate-100 dark:bg-background-dark rounded-full p-1">
                    <button 
                      ...
                      className="size-7 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-slate-800 transition-colors"
                    >
                      <Minus size={14} />
                    </button>
                    ...
                    <button 
                      onClick={() => updateQuantity(item.productId, 1)}
                      className="size-7 bg-white dark:bg-slate-700 flex items-center justify-center rounded-full shadow-sm"
                    >
                      <Plus size={14} className="text-primary dark:text-white" />
                    </button>
                  </div>
                  <p className="text-[10px] font-bold text-slate-400">Sub: R$ {(item.price * item.quantity).toFixed(2)}</p>
                </div>
              </div>
            ))}
            ...
            {saleItems.length === 0 && !isSearching && (
              <button 
                ...
                className="w-full py-8 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:border-primary/50 hover:text-primary transition-all"
              >
              ...
              <div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto no-scrollbar">
                {filteredProducts.map(product => (
                  <button
                    ...
                    className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-left transition-colors"
                  >
                    <img src={product.image} className="size-10 rounded object-cover" alt="" />
                    <div className="flex-1">
                      <p className="text-sm font-bold">{product.name}</p>
                      <p className="text-xs text-slate-500">R$ {product.price.toFixed(2)} • Estoque: {product.stock}</p>
                    </div>
        ...
        <section className="p-4 space-y-6">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Pagamento</h3>
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all ${
                    paymentMethod === method.id
                      ...
                      : "bg-white dark:bg-surface-dark border-transparent text-slate-500"
                  }`}
                >
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold text-slate-400 uppercase">Data da Venda</p>
            <div className="relative">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <Input 
                type="date" 
                className="pl-10 h-12 bg-white dark:bg-surface-dark border-none" 
                value={saleDate}
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-background-light dark:bg-background-dark border-t border-slate-200 dark:border-slate-800 p-4 pb-8 z-50">
        <div className="max-w-4xl mx-auto w-full flex flex-col gap-4">
          <div className="flex items-center justify-between px-1">
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Total da Venda</p>
            <p className="text-2xl font-black tracking-tight">R$ {total.toFixed(2)}</p>
          </div>
          <Button 
            onClick={handleRegisterSale}
            disabled={saleItems.length === 0}
            className="w-full h-14 bg-primary hover:bg-blue-600 text-white font-bold text-lg rounded-2xl shadow-lg shadow-primary/20 transition-all active:scale-[0.98]"
          >
            Registrar Venda
          </Button>
        </div>
